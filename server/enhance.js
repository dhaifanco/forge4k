...
function status() {
  ...
  return { upscale: fs.existsSync(path.join(root, 'esrgan', 'realesrgan-ncnn-vulkan.exe')) && fs.existsSync(path.join(root, 'esrgan', 'models', 'realesrgan-x4plus.bin')),
    interpolate: fs.existsSync(path.join(root, 'rife', 'rife-ncnn-vulkan.exe')) && fs.existsSync(path.join(root, 'rife', 'rife-v4.6', 'flownet.bin')) };
}
...
function plan(probe, preset, value, preview = false) {
  ...
  // Include the 4x network output, source PNGs and interpolated frames, not just the final MP4.
  const frames = Math.ceil(duration * Math.max(v.fps, fps)) + 2;
  const pixels = v.width * v.height;
  const scratchBytes = Math.ceil(frames * pixels * 4 * (opt.scale > 1 ? 18 : 3) + 256 * 1024 ** 2);
...
async function render({ input, output, probe, preset, adv, job, tempRoot, bins }) {
  ...
  async function run(bin, args, isFfmpeg = false) {
    if (job.cancelRequested) throw new Error('Cancelled');
    const result = await ff.runBin(bin, args, { progress: isFfmpeg, timeoutMs: 24 * 60 * 60 * 1000,
  ...
  try {
    ...
    await run(bins.ffmpegPath, ['-hide_banner','-nostdin','-y','-i',input,'-t',String(p.duration),'-an','-vf',filters.join(','),'-start_number','1',path.join(source,'%08d.png')], true);
    let images = source;
    ...
    if (p.fps > p.sourceFps + .01) {
      ...
      await run(path.join(engineRoot(),'rife','rife-ncnn-vulkan.exe'), ['-i',images,'-o',interpolated,'-m',path.join(engineRoot(),'rife','rife-v4.6'),'-n',String(desired),'-j','1:1:1','-f','%08d.png']);
    ...
    if (p.scale > 1) {
      ...
      await run(path.join(engineRoot(),'esrgan','realesrgan-ncnn-vulkan.exe'), ['-i',images,'-o',enhanced,'-m',path.join(engineRoot(),'esrgan','models'),'-n','realesrgan-x4plus','-s','4','-t','128','-j','1:1:1','-f','png']);
    ...
    const args = ['-hide_banner','-nostdin','-y','-framerate',String(p.fps),'-start_number','1','-i',path.join(images,'%08d.png'),'-i',input,'-map','0:v:0','-map','1:a:0?','-t',String(p.duration),'-vf',`scale=${p.width}:${p.height}:flags=lanczos,tpad=stop_mode=clone:stop_duration=1,format=yuv420p`,'-c:v',encoder.encoder];
    ...
    args.push('-c:a','aac','-b:a','192k','-ar','48000','-ac','2','-map_metadata','-1','-colorspace','bt709','-color_primaries','bt709','-color_trc','bt709','-movflags','+faststart',output);
    return await run(bins.ffmpegPath,args,true);
